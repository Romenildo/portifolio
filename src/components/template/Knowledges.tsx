...
export default function Knowledges() {
  ...
  function showDetail(){
    return(
      <ItemDetail title={currentItem?.name} description={currentItem?.description}></ItemDetail>
    )
  }
  ...
  return (
    <div className={styles.knowledges}>
      ...
      <MenuItems handleClick={handleClick} option1="Back-end" option2="Front-end" option3="Conhecimentos"></MenuItems>

      <ul className={styles.content}>
        { currentItems.length > 0 ?(
          currentItems.map((e:any)=>(
            <li className={activeItemIndex === e.id ? styles.active : ""} key={e.id} onClick={() => activeItem(e)}>{e.name}</li>
          ))
        ):(
